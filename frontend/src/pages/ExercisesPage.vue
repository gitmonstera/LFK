    ...
    <q-dialog v-model="showExerciseDialog">
      <q-card style="min-width: 350px; max-width: 500px; width: 100%;">
        ...
        <q-card-actions align="right">
          ...
          <q-btn flat label="Начать тренировку" color="primary" @click="startFromDialog" />
        </q-card-actions>
      </q-card>
    </q-dialog>
  </q-page>
...
// Переход к упражнению из диалога
...
// Начать упражнение - передаем все данные через query
const startExercise = (exercise) => {
  showExerciseDialog.value = false
  router.push({
    path: `/profile/exercise/${exercise.exercise_id}`,
    query: {
      name: exercise.name,
      description: exercise.description || '',
      instructions: JSON.stringify(exercise.instructions || []),
      target_muscles: JSON.stringify(exercise.target_muscles || []),
      duration_seconds: exercise.duration_seconds || 0,
      category: exercise.category || '',
      category_icon: exercise.category_icon || '',
      category_color: exercise.category_color || '#667eea',
