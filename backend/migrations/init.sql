...
-- =====================================================
-- Добавление упражнения для шеи (ИСПРАВЛЕННАЯ ВЕРСИЯ)
-- =====================================================

-- Добавляем новую категорию "Шея" если её нет
INSERT INTO exercise_categories (name, description, icon, color) VALUES
    ('Шея', 'Упражнения для шейного отдела позвоночника', '🦒', '#E74C3C')
    ON CONFLICT (name) DO NOTHING;

-- Добавляем упражнение "Наклоны и повороты шеи"
INSERT INTO exercises (
    ...
    applicable_codes
             ...
             'Плавные наклоны и повороты головы для восстановления подвижности шейного отдела позвоночника. Упражнение помогает снять напряжение, улучшить кровообращение и восстановить подвижность после травм и заболеваний шейного отдела.',
             (SELECT id FROM exercise_categories WHERE name = 'Шея'),
             2,
             ARRAY['Грудино-ключично-сосцевидная мышца', 'Лестничные мышцы', 'Трапециевидная мышца (верхняя часть)', 'Подзатылочные мышцы'],
             ARRAY[
                 'Встаньте прямо, смотрите перед собой',
             'Медленно наклоните голову влево, стараясь приблизить ухо к плечу',
             'Задержитесь в этом положении на 2-3 секунды',
             'Вернитесь в исходное положение',
             'Наклоните голову вправо, задержитесь на 2-3 секунды',
             'Вернитесь в исходное положение',
             'Медленно поверните голову влево, стараясь увидеть плечо',
             'Задержитесь на 2-3 секунды',
             'Вернитесь в исходное положение',
             'Поверните голову вправо, задержитесь на 2-3 секунды',
             'Вернитесь в исходное положение',
             'Повторите весь цикл 3-5 раз'
                 ],
             120,
             5.0,
             ARRAY[
                 'M47.1–M47.9',      -- Спондилез шейного отдела, спондилез с миелопатией
             'M50.0–M50.1',      -- Поражение межпозвоночных дисков шейного отдела с радикулопатией и миелопатией
             'M53.0–M53.1',      -- Цервикокраниальный синдром, шейно-плечевой синдром
             'M54.0–M54.2',      -- Плекситы, радикулопатии, цервикалгии
             'G44.2–G44.4',      -- Головные боли напряжения, после травм
             'G54.0–G54.2',      -- Поражения нервных корешков и сплетений шейного отдела
             'S13.0–S13.4',      -- Вывихи, растяжения связок шейного отдела
             'S16',              -- Травма мышц и сухожилий шеи
             'M99.0–M99.1',      -- Сегментарная и соматическая дисфункция шейного отдела
             'I63–I64',          -- Последствия инсульта (постинсультная реабилитация)
             'G35',              -- Рассеянный склероз
             'G81.0–G81.9',      -- Гемиплегия
             'Z50.0',            -- Лечебная физкультура (основной код назначения)
             'Z50.3',            -- Реабилитация наркологических больных
             'Z50.9'  ]
         )
    ...
    name = EXCLUDED.name,
                            ...
                            applicable_codes = EXCLUDED.applicable_codes;